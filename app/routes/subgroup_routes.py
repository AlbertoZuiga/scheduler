    ...
    return {
        **preview,
        "groups": [
            {
                **group_data,
                "members": [{**member, "email": ""} for member in group_data.get("members", [])],
            }
...
def generate(group_id):
    ...
    try:
        ...
        config["required_membership_categories"] = [
            str(category).strip()
            for category in required_membership_categories
...
def export(group_id):
    ...
    try:
        ...
        response.headers["Content-Disposition"] = (
            f"attachment; filename=subgroups_group_{group_id}.csv"
        )
...
def index(group_id):
    ...
    subgroup_member_user_ids = {
        subgroup.id: {member.user_id for member in subgroup.members} for subgroup in all_subgroups
    }
    ...
    group_members = (
        get_group_members_sorted(group_id) if can_view_all or editable_subgroup_ids else []
    )
    assigned_user_ids = {
        member.user_id for subgroup in all_subgroups for member in subgroup.members
    }
...
def create_manual(group_id):
    ...
    try:
        ...
        current_app.logger.exception(
            "Error de BD al crear subgrupo manual en el grupo %s", group_id
        )
...
@subgroup_bp.route(
    "/groups/<int:group_id>/subgroups/<int:subgroup_id>/members/add", methods=["POST"]
)
@login_required
def add_member(group_id, subgroup_id):
    ...
    try:
        ...
        scheduler_db.session.commit()
        flash(
            f"{display_name(group_member.user, with_email=False)} "
...
@subgroup_bp.route(
    "/groups/<int:group_id>/subgroups/<int:subgroup_id>/members/<int:user_id>/remove",
    methods=["POST"],
)
@login_required
...
@subgroup_bp.route(
    "/groups/<int:group_id>/subgroups/<int:subgroup_id>/members/<int:user_id>/move",
    methods=["POST"],
)
@login_required
def move_member(group_id, subgroup_id, user_id):
    ...
    try:
        ...
        scheduler_db.session.commit()
        flash(
            f"{display_name(user, with_email=False)} fue movido de "
