...
class SubGroup(SoftDeleteMixin, scheduler_db.Model):
    ...
    parent_group_id = scheduler_db.Column(
    ...
    meta = scheduler_db.Column(
        scheduler_db.JSON, default=dict
    )  # Almacena compatibilidad promedio, reglas cumplidas, etc.
    created_at = scheduler_db.Column(scheduler_db.DateTime, default=_utcnow, nullable=False)

    # Relaciones
    parent_group = scheduler_db.relationship(
        "Group",
        backref=scheduler_db.backref(
            "subgroups", lazy="dynamic", cascade="all, delete-orphan", passive_deletes=True
        ),
    )
...
class SubGroupMember(SoftDeleteMixin, scheduler_db.Model):
    ...
    id = scheduler_db.Column(scheduler_db.Integer, primary_key=True)
    subgroup_id = scheduler_db.Column(
        scheduler_db.Integer,
        scheduler_db.ForeignKey("subgroups.id", ondelete="CASCADE"),
        nullable=False,
    )
    user_id = scheduler_db.Column(
        scheduler_db.Integer, scheduler_db.ForeignKey("user.id", ondelete="CASCADE"), nullable=False
    )
...
class DivisionJob(SoftDeleteMixin, scheduler_db.Model):
    ...
    created_by = scheduler_db.Column(
        scheduler_db.Integer, scheduler_db.ForeignKey("user.id", ondelete="SET NULL"), nullable=True
    )
    config_json = scheduler_db.Column(
        scheduler_db.JSON, nullable=False
    )  # Configuración de entrada (num_groups, rules, etc.)
    result_json = scheduler_db.Column(
        scheduler_db.JSON, nullable=True
    )  # Resultado completo (preview)
    status = scheduler_db.Column(
        scheduler_db.String(50), default="pending", nullable=False
    )  # pending, confirmed, undone
    ...
    parent_group = scheduler_db.relationship(
        "Group",
        backref=scheduler_db.backref(
            "division_jobs", lazy="dynamic", cascade="all, delete-orphan", passive_deletes=True
        ),
    )
    # `created_by` es SET NULL: el job sobrevive al borrado de quien lo lanzó.
    creator = scheduler_db.relationship(
        "User",
        backref=scheduler_db.backref("division_jobs_created", lazy="dynamic", passive_deletes=True),
    )
